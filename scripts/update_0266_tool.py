#!/usr/bin/env python3
"""Transactional installer and rollback utility for AFI-OS 0.2.66.

The release package is expected to contain:

    UPDATE-AFI-OS-0.2.66.command
    update_0266_tool.py
...
def install_update(target, manifest_path, payload_root):
    ...
    backup_directory = create_unique_backup_directory(target, "update-0.2.66")
    ...
    try:
        ...
        announce("Đang chạy migration 0.2.66…")
    ...
    announce("Cập nhật AFI-OS 0.2.66 thành công; dữ liệu cũ đã được giữ nguyên.")
...
def load_update_backup(backup_directory):
    ...
    if manifest.get("update_version") != UPDATE_VERSION:
        raise UpdateError("Backup không thuộc bản 0.2.66")
...
def latest_installed_backup(target):
    ...
    if backup_root.is_dir() and not backup_root.is_symlink():
        for path in backup_root.glob("update-0.2.66-*"):
    ...
    if not candidates:
        raise UpdateError("Không tìm thấy update-0.2.66 backup đang ở trạng thái INSTALLED")
    candidates.sort(key=lambda item: (item[0], item[1].name), reverse=True)
    return candidates[0][1], candidates[0][2]


def snapshot_current_state_for_emergency(target, source_manifest):
    emergency = create_unique_backup_directory(target, "emergency-before-rollback-0.2.66")
    database_path = safe_target_path(target, DEFAULT_DATABASE_PATH, allow_database=True)
...
def rollback_update(target, backup_directory=None):
    ...
    stop_result = stop_application(target)
    announce("Đang tạo emergency backup của trạng thái 0.2.66 hiện tại…")
    ...
    try:
        announce("Đang khôi phục database và code trước update 0.2.66…")
        ...
        announce("Rollback lỗi; đang khôi phục lại trạng thái 0.2.66 từ emergency backup…")
        try:
        ...
        if isinstance(exc, KeyboardInterrupt):
            raise UpdateError("Rollback bị hủy; trạng thái 0.2.66 đã được phục hồi")
        if isinstance(exc, UpdateError):
            raise
        raise UpdateError("Rollback thất bại: {0}".format(exc))

    announce("Rollback 0.2.66 thành công.")
...
def build_parser():
    ...
    parser = argparse.ArgumentParser(description="AFI-OS 0.2.66 safe updater")
    subparsers = parser.add_subparsers(dest="command", required=True)

    install_parser = subparsers.add_parser("install", help="Install AFI-OS 0.2.66")
    ...
    rollback_parser = subparsers.add_parser("rollback", help="Rollback latest AFI-OS 0.2.66")
