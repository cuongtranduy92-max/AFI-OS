...
UPDATER_PATH = Path(__file__).resolve().parents[1] / "scripts" / "update_02107_tool.py"
SPEC = importlib.util.spec_from_file_location("update_02107_tool", UPDATER_PATH)
...
def test_install_and_rollback_preserve_database_and_version(
    ...
    with sqlite3.connect(target / "data/afi_os.db") as connection:
        ...
        connection.execute("INSERT INTO alembic_version VALUES ('e91f4d7a2c18')")
    ...
    def fake_upgrade(_target, database, _log):
        with sqlite3.connect(database) as connection:
            connection.execute("UPDATE alembic_version SET version_num = '4f7c2a91d5e0'")
        return {
