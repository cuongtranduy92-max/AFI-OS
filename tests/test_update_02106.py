...
UPDATER_PATH = Path(__file__).resolve().parents[1] / "scripts" / "update_02106_tool.py"
SPEC = importlib.util.spec_from_file_location("update_02106_tool", UPDATER_PATH)
...
def test_install_and_rollback_preserve_database_and_version(
    ...
    with sqlite3.connect(target / "data/afi_os.db") as connection:
        ...
        connection.execute("INSERT INTO alembic_version VALUES ('b84d0e26c104')")
    ...
    def fake_upgrade(_target, database, _log):
        with sqlite3.connect(database) as connection:
            connection.execute("UPDATE alembic_version SET version_num = 'e91f4d7a2c18'")
        return {
    ...
    assert updater.installed_version(target) == "0.2.106"
    with sqlite3.connect(target / "data/afi_os.db") as connection:
    ...
    assert updater.installed_version(target) == "0.2.105"
    with sqlite3.connect(target / "data/afi_os.db") as connection:
        assert connection.execute("SELECT value FROM preserved_rows").fetchone() == ("keep-me",)
