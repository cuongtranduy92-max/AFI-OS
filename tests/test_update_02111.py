...
UPDATER_PATH = Path(__file__).resolve().parents[1] / "scripts" / "update_02111_tool.py"
SPEC = importlib.util.spec_from_file_location("update_02111_tool", UPDATER_PATH)
...
def test_updater_install_and_rollback_preserve_database(
    ...
    def fake_upgrade(_target, database, _log):
        with sqlite3.connect(database) as connection:
            connection.execute("UPDATE alembic_version SET version_num = '93d7e5a2b1c4'")
    ...
    assert updater.installed_version(target) == "0.2.111"
    with sqlite3.connect(target / "data/afi_os.db") as connection:
