...
UPDATER_PATH = Path(__file__).resolve().parents[1] / "scripts" / "update_02103_tool.py"
SPEC = importlib.util.spec_from_file_location("update_02103_tool", UPDATER_PATH)
...
def test_install_and_rollback_preserve_database_and_version(
        ...
        json.dumps(
            {
                ...
                "allowed_from_versions": ["0.2.102"],
                "expected_migration_head": "a73c9e15b642",
    ...
    assert updater.installed_version(target) == "0.2.102"
    with sqlite3.connect(target / "data/afi_os.db") as connection:
        assert connection.execute("SELECT value FROM preserved_rows").fetchone() == ("keep-me",)
