...
TOOL="$PACKAGE_ROOT/update_02100_tool.py"
...
if [[ "$STATUS" -eq 0 ]]; then
  ...
  if command -v launchctl >/dev/null 2>&1 && \
    ...
    echo "[AFI-OS] Dịch vụ 24/7 đã tự khởi động lại bản 0.2.100."
    [[ "${AFI_OS_SKIP_START:-0}" == "1" ]] || open "http://127.0.0.1:8765"
  elif [[ "${AFI_OS_SKIP_START:-0}" != "1" && -x "$TARGET/START-AFI-OS.command" ]]; then
